...
while True:
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            x1,y1,x2,y2 = int(x1), int(y1), int(x2), int(y2) # Convert tensor values into Integer
            w, h = x2-x1, y2-y1
            cvzone.cornerRect(img, (x1, y1, w, h), t=5, rt=2, colorR=(255, 0, 255), colorC=(0, 255, 0)) # t=Thickness, rt=Recttangle Thickness, Color of Rectangle, Color of Corner

            # Confidece
            confidence = math.ceil((box.conf[0]*100))/100
            # Classname
            cls= int(box.cls[0])
            cvzone.putTextRect(img, f'{classNames[cls]} {confidence}', (max(0,x1), max(30, y1)), scale=1, thickness=2) # Classification by label
